...
function ImportDirectory:getModuleName(descriptoroffset)
  local namerva = self.buffer:readType(descriptoroffset + 0x0C, pref.datatype.UInt32_LE)
...
function ImportDirectory:getImportName(thunkdata)
  ...
  return self.buffer:readString(thunkdataoffset + pref.datatype.sizeof(pref.datatype.UInt16_LE)) -- Skip Ordinal
end
...
function ImportDirectory:parseThunk(thunkstruct, thunkoffset)
  local i = 0
  local pthunk = Pointer(thunkoffset, pref.datatype.UInt32_LE, self.buffer)
  
  while pthunk[i] ~= 0 do
    local importname = self:getImportName(pthunk[i])
    thunkstruct:addField(pref.datatype.UInt32_LE, importname)
    i = i + 1
  end
end
...
function ImportDirectory:parse()
  ...
  local oft = buffer:readType(offset, pref.datatype.UInt32_LE)
  local ft = buffer:readType(offset + 0x10, pref.datatype.UInt32_LE)
  
  while (oft ~= 0) or (ft ~= 0) do
    local modulename = self:getModuleName(offset)
    local descriptor = importdirectory:addStructure(modulename)
    descriptor:addField(pref.datatype.UInt32_LE, "OriginalFirstThunk")
    descriptor:addField(pref.datatype.UInt32_LE, "TimeDateStamp")
    descriptor:addField(pref.datatype.UInt32_LE, "ForwarderChain")
    descriptor:addField(pref.datatype.UInt32_LE, "Name")
    descriptor:addField(pref.datatype.UInt32_LE, "FirstThunk")
    
    self:parseOFT(oft, modulename)
    self:parseFT(ft, modulename)
    
    offset = offset + descriptor.size
    oft = buffer:readType(offset, pref.datatype.UInt32_LE)
    ft = buffer:readType(offset + 0x10, pref.datatype.UInt32_LE)
  end
end
