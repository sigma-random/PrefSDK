...
function ExportDirectory:__ctor(databuffer, formattree, section, directoryoffset)
...
function ExportDirectory:parseExportedFunctions(exportdirectory)
  local databuffer, formattree = self.databuffer, self.formattree
  ...
  local pfunc = Pointer(funcoffset, DataType.UInt32_LE, databuffer)
  local pnames = Pointer(namesoffset, DataType.UInt32_LE, databuffer)
  local pnameord = Pointer(nameordoffset, DataType.UInt16_LE, databuffer)
    ...
    if pfunc[i] ~= 0 then
      ...
      for j = 0, exportdirectory.NumberOfNames:value() - 1 do        
        if pnameord[j] == i then
          ...
          exportedfunctions:addField(DataType.UInt32_LE, databuffer:readString(nameoffset))
          break
        end
      end
      
      if not found then
        exportedfunctions:addField(DataType.UInt32_LE, string.format("Ordinal_%04X", tonumber(ordinal)))
      end
    end
  end
end
